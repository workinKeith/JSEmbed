...
var jsembed = function()
{
    ...
    var SCALETYPE_FILL_TO_PORTRAIT = "filltop"; // Fills the browser window with the canvas, accounts for portrait
	...
	var _allowPortrait = false; // Whether or not we support portrait flipping
	var _isPortrait = false; // Whether or not we're displaying in portrait
	...
	*/      
	...
	function scaleSet (pScale)
	{
	    ...
	    if(parameters.scaletype!=SCALETYPE_NONE)
	    {
	    	...
	    	} else if(parameters.scaletype==SCALETYPE_FILL_TO)
	    	{
	    		...
	    		d.style.height = tFinalH + "px"; 
	    		d.style.left = ((getBrowserWidth()/2) - (tFinalW/2)) + "px";
	    	} else if ( parameters.scaletype == SCALETYPE_FILL_TO_PORTRAIT ) {

	    		tFinalW = Math.round(widthMax*pScale*1000)/1000;
	    		tFinalH = Math.round(heightMax*pScale*1000)/1000;
	    		if ( _isPortrait ) {	    			
	    			if ( getBrowserHeight() > tFinalW ) {
		    			d.style.top = (((getBrowserHeight()/2) - (tFinalW/2))) + "px";
		    			d.style.height = tFinalW + "px";
		    		} else { 
		    			d.style.top = "0px";
		    			d.style.height = getBrowserHeight() + "px";
		    			tFinalW = getBrowserHeight();
		    		}
		    		if ( getBrowserWidth() > tFinalH ) {
		    			d.style.left = (((getBrowserWidth()/2) - (tFinalH/2))) + "px";
		    			d.style.width = tFinalH + "px";		    			
		    		} else {
		    			d.style.left = "0px";
		    			d.style.width = getBrowserWidth() + "px";
		    			tFinalH = getBrowserWidth();					
		    		} 			
	    		} else {
	    ...
	    _canvasScale	= pScale;	    
	    if ( _isPortrait ) {
	    	_canvasWidth 	= getBrowserHeight();
	    	_canvasHeight	= getBrowserWidth();
	    } else {
         ...
         if (parameters.scaletype==SCALETYPE_FIT_HORIZONTAL){
        	 ...
        	 _allowPortrait = true;
     	 	if ( tHeight < tWidth ) {
     	 		...
     	 		if ( tWidth/heightIdeal < tHeight/widthIdeal ) {
     	 			tScale = tWidth/heightIdeal;
     	 		} else {
     	 			tScale = tHeight/widthIdeal;
     	 		}
     	 		setPortrait(true);
     	 	}
          } else {
	...
	/*
	 * Setting the isPortrait flag
	 */
	function setPortrait(pPortrait) {
		if ( _allowPortrait == false ) {
			_isPortrait = false;
			return;
		}		
		_isPortrait = pPortrait;
	}
	
	/*
     * Get whether or not we're in portrait mode
     */
    ...
    /*
     *  Get whether or not we're allowed in portrait mode
     */
    function getAllowPortrait() {     	
    	return _allowPortrait;
    }
	 ...
	 */
	return {
		allowPortrait : function() {
			return getAllowPortrait();
		},
		isPortrait : function() {
